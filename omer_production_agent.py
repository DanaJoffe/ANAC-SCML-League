...
from scml.scml2020 import SCML2020Agent, PredictionBasedTradingStrategy, NO_COMMAND, DecentralizingAgent, \
    BuyCheapSellExpensiveAgent, IndDecentralizingAgent, MovingRangeAgent, MovingRangeNegotiationManager
...
class OmerProductionStrategyAgent(ProductionStrategy):
    ...
    def _load_omer_production_args(self):
        if not hasattr(self, '_production_classifier'):
            self._production_classifier, self._vectorizer = pickle.load(open("production_binary_classifier.pkl", "rb"))
        ...
        if not hasattr(self, '_features'):
            self._features = [
                {
                    "n_agents": len("|".join(["|".join(x) for x in self.awi.all_consumers]).split("|")) - 1,
                    "step": -1, "steps_to_end": self.awi.state.n_steps - self.awi.current_step,
                    "relative_step": 0,
                    "input_product": self.awi.my_input_product, "output_product": self.awi.my_input_product + 1,
                    ...
                    "n_buy": 0, "n_buy_price_mean": 0, "n_buy_price_std": 0, "n_buy_quantity_mean": 0,
                    "n_buy_quantity_std": 0, "n_buy_time_mean": 0,
                    "n_sell": 0, "n_sell_price_mean": 0, "n_sell_price_std": 0, "n_sell_quantity_mean": 0,
                    "n_sell_quantity_std": 0, "n_sell_time_mean": 0,
                }
            ]
    ...
    def _save_feature_state(self, signed):
        ...
        vec = {
            ...
            "n_buy_price_mean": self._mean([contract.agreement['unit_price'] for contract in signed_buy if contract]),
            "n_buy_price_std": self._std([contract.agreement['unit_price'] for contract in signed_buy if contract]),
            "n_buy_quantity_mean": self._mean([contract.agreement['quantity'] for contract in signed_buy if contract]),
            "n_buy_quantity_std": self._std([contract.agreement['quantity'] for contract in signed_buy if contract]),
            "n_buy_time_mean": self._mean([contract.agreement['time'] - self.awi.current_step for contract in signed_sell if contract]),
            "n_sell": len(signed_sell),
            "n_sell_price_mean": self._mean([contract.agreement['unit_price'] for contract in signed_sell if contract]),
            "n_sell_price_std": self._std([contract.agreement['unit_price'] for contract in signed_sell if contract]),
            "n_sell_quantity_mean": self._mean([contract.agreement['quantity'] for contract in signed_sell if contract]),
            "n_sell_quantity_std": self._std([contract.agreement['quantity'] for contract in signed_sell if contract]),
            "n_sell_time_mean": self._mean([contract.agreement['time'] - self.awi.current_step for contract in signed_sell if contract]),
        }
        self._features.append(vec)

    def _produce_by_prediction(self):
        instance = {}
        for (prev_k, prev_v), (next_k, next_v) in zip(self._features[-2].items(), self._features[-1].items()):
            instance["prev_" + prev_k] = prev_v
            instance["next_" + next_k] = next_v
        to_prodeuce = self._production_classifier.predict(self._vectorizer.transform(instance)).item()
    ...
    def on_contract_breached(self, contract: Contract, breaches: List[Breach], resolution: Optional[Contract]) -> None:
        ...
        if self.id == contract.annotation['seller']:
            self.omer_output += int(contract.agreement["quantity"] * (1.0 - max(b.level for b in breaches)))
        else:
            self.omer_input += int(contract.agreement["quantity"] * (1.0 - max(b.level for b in breaches)))
