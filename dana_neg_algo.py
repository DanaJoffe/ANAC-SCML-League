...
class CalcTrustworthiness(object):
    ...
    def eval_trustworthiness(self, u, offer: Optional["Outcome"]):
        ...
        rival_agent_id = self.ami.annotation['buyer'] if self.controller.is_seller else self.ami.annotation['seller']
        ...
        if self.last_step:
            ...
            breach_level = sum(fr.breach_level for step, fr in financial_rep.items()) / len(financial_rep)
            breach_prob = sum(fr.breach_prob for step, fr in financial_rep.items()) / len(financial_rep)
        ...
        trustworthiness = self.breach_level_w * (1 - breach_level) + self.breach_prob_w * (1 - breach_prob)
