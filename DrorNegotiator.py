...
class DrorMeanERPStrategy(ExecutionRatePredictionStrategy):
    ...
    def init(self):
        ...
        for item in self.awi.my_consumers:
            self.sell_total_quantities[item] = {"rate": self._execution_fraction,
                                                "quantity": self._total_quantity}
        for item in self.awi.my_suppliers:
            self.buy_total_quantities[item] = {"rate": self._execution_fraction,
                                                "quantity": self._total_quantity}
    ...
    def update_dicts(self, contract, q):
        if self.id == contract.annotation['buyer']:
            ...
            if seller:
                ...
                seller['quantity'] += q
                seller['rate'] = (seller['rate']*seller_old_quantity + q) / seller['quantity']
        else:
            buyer = self.buy_total_quantities.get(contract.annotation['buyer'])
            if buyer:
                buyer_old_quantity = buyer['quantity']
                buyer['quantity'] += q
                buyer['rate'] = (buyer['rate']*buyer_old_quantity + q) / buyer['quantity']
...
class DrorStepNegotiationManager(MeanERPStrategy, DrorNegotiationManager):
    """
    ...
    def __init__(
        ...
        # TODO super().__init__(*args, execution_fraction=0.3, **kwargs)
    ...
    def _start_negotiations(
        ...
        if sell:
            # TODO:
            # execution_fraction = mean([item[1].get('rate') for item in self.sell_total_quantities.items()])
            ...
            # TODO
            # execution_fraction = mean([item[1].get('rate') for item in self.buy_total_quantities.items()])
            # expected_quantity = int(math.floor(qvalues[1] * execution_fraction))
            expected_quantity = int(math.floor(qvalues[1] *self._execution_fraction))
...
class DrorStepAgent(DrorStepNegotiationManager,
    ...
    def set_param(self, exec_fraction, production_cost_factor=1):
        self.production_cost_factor = production_cost_factor # TODO=1.03
        self._execution_fraction = exec_fraction

    def acceptable_unit_price(self, step: int, sell: bool) -> int:
        production_cost = np.max(self.awi.profile.costs[:, self.awi.my_input_product])
        if sell:
            return self.production_cost_factor * (production_cost + self.input_cost[step])
